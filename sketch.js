...
var namee="Anonymous";
var loggedin = false;
var CM1, CM2, CM3, CM4, CM5, CM6, CM7, CM8, CM9, CM10, CM11, CM12, CM13, CM14, CM15;
var reported = false;
...
function setup(){
  ...
  icon1.visible=false;
  icon2.visible=false;
  a1 = createSprite(680, 320, 200, 200);
  a1.addImage(aimg1);
  a1.scale = 0.045;
  a1.visible = false;
  a1.depth = 2;
  a2 = createSprite(680, 320, 200, 200);
  a2.addImage(aimg2);
  a2.scale = 0.045;
  a2.visible = false;
  a2.depth = 2;
  a3 = createSprite(680, 320, 200, 200);
  a3.addImage(aimg3);
  a3.scale = 0.045;
  a3.visible = false;
  a3.depth = 2;
  a4 = createSprite(680, 320, 200, 200);
  a4.addImage(aimg4);
  a4.scale = 0.045;
  a4.visible = false;
  a4.depth = 2;
  a5 = createSprite(680, 320, 200, 200);
  a5.addImage(aimg5);
  a5.scale = 0.045;
  a5.visible = false;
  a5.depth = 2;
  a6 = createSprite(680, 320, 200, 200);
  a6.addImage(aimg6);
  a6.scale = 0.045;
  a6.visible = false;
  a6.depth = 2;
  a7 = createSprite(680, 320, 200, 200);
  a7.addImage(aimg7);
  a7.scale = 0.045;
  a7.visible = false;
  a7.depth = 2;
  a8 = createSprite(680, 320, 200, 200);
  a8.addImage(aimg8);
  a8.scale = 0.045;
  a8.visible = false;
  a8.depth = 2;
  a9 = createSprite(680, 320, 200, 200);
  a9.addImage(aimg9);
  a9.scale = 0.045;
  a9.visible = false;
  a9.depth = 2;
  a10 = createSprite(680, 320, 200, 200);
  a10.addImage(aimg10);
  a10.scale = 0.045;
  a10.visible = false;
  a10.depth = 2;
  close = createSprite(815, 243, 200, 200);
  close.addImage(closeimg);
  close.scale = 0.15;
  close.visible = false;
  close.x=displayWidth+500;
  close.depth = 10;
...
function draw(){
  ...
  if(adpos=="regular"){
    ...
    if(close.visible==true){
      ...
      close.x=815;
      close.y=243;
    } else if (close.visible==false){
      console.log("bye");
      close.x=displayWidth+500;
    close.y=315;
    }
    } else if (adpos=="odd") {
      a1.x=displayWidth/2-70;
      a1.y=395;
      a2.x = displayWidth/2-70;
      a2.y = 392;
      
      a3.x = displayWidth/2-70;
      a3.y = 392;
      
      a4.x = displayWidth/2-70;
      a4.y = 392;
      
      a5.x = displayWidth/2-70;
      a5.y = 392;
      
      a6.x = displayWidth/2-70;
      a6.y = 392;
      
      a7.x = displayWidth/2-70;
      a7.y = 392;
      
      a8.x = displayWidth/2-70;
      a8.y = 392;
      
      a9.x = displayWidth/2-70;
      a9.y = 392;
      
      a10.x = displayWidth/2-70;
      a10.y = 392;
      close.x=displayWidth/2+65;
      close.y=315;
      if(close.visible==true){
        console.log("hi");
        close.x=displayWidth/2+65;
  ...
  if(hide6==false){
    text(CM1, displayWidth/2-635, 144);
    text(CM2, displayWidth/2-635, 169);
    text(CM3, displayWidth/2-635, 194);
    text(CM4, displayWidth/2-635, 219);
    text(CM5, displayWidth/2-635, 244);
    text(CM6, displayWidth/2-635, 269);
    
    text(CM7, displayWidth/2-635, 294);
    
    text(CM8, displayWidth/2-635, 489);
    text(CM9, displayWidth/2-635, 514);
    text(CM10, displayWidth/2-635, 539);
    text(CM11, displayWidth/2-635, 564);
    text(CM12, displayWidth/2-635, 589);
    text(CM13, displayWidth/2-635, 614);
    text(CM14, displayWidth/2-635, 639);
    text(CM15, displayWidth/2-635, 664);
  ...
  if(mousePressedOver(report)){
    ...
    //    {
     //     confirmButtonColor: '#8CD4F5', title: `Successfully Reported`,
      ...
      swal({ confirmButtonColor: '#CD853F', background:'#FF6B6B',   
       title: `Successfully Reported`, text: "You have successfully reported inappropriate content.", 
       type: 'success', 
       imageSize: "306x220", confirmButtonText: "Done", },    function(isConfirm) {
        if (isConfirm) {
...
// Define a named function to fetch date and time
function fetchDateTime() {
  ...
  fetch('http://worldtimeapi.org/api/ip')
    ...
    .then(data => {
      ...
      rawDateTime = new Date(data.datetime);

      // Extract the date
      date = rawDateTime.toDateString();

      // Extract the time and round it to two decimal places
      time = rawDateTime.toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit' });

      // Use the date and time variables for further processing or display
...
// Call the function to fetch the date and time
